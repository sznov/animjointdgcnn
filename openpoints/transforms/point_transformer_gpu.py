...
class PointCloudCenterAndNormalize(object):
    ...
    def __call__(self, data):
        state = self.fit(data)
        data = self.transform(data, state)
    ...
    def fit(self, data):
        ...
        if self.centering:
            state['orig_mean'] = torch.mean(data, axis=0, keepdims=True)

        if self.normalize:
            state['orig_m'] = torch.max(torch.sqrt(torch.sum(data ** 2, axis=-1, keepdims=True)), axis=0, keepdims=True)[0]

        return state
    
    def transform(self, data, state):
        data_pos = data['pos'] if hasattr(data, 'keys') else data
        
        if hasattr(data, 'keys'):
            result = {}
            if self.append_xyz:
                result['heights'] = data_pos - torch.min(data_pos)
            else:
                height = data_pos[:, self.gravity_dim:self.gravity_dim + 1]
                result['heights'] = height - torch.min(height)

        if self.centering and 'orig_mean' in state:
            data_pos = data_pos - state['orig_mean']

        if self.normalize and 'orig_m' in state:
            data_pos = data_pos / state['orig_m']
    ...
    def inverse_transform(self, data, state):
        if self.normalize and 'orig_m' in state:
            data = data * state['orig_m']
        
        if self.centering and 'orig_mean' in state:
            data = data + state['orig_mean']
