...
class DGCNN(nn.Module):
    ...
    def forward(self, pts, features=None):
        ...
        neighbourhoods = self.knn(pts)
        feats = [self.head(features, neighbourhoods)]
        for i in range(self.n_blocks - 2):
            feats.append(self.backbone[i](feats[-1], neighbourhoods)) # TODO Don't pass neighbourhoods if we want them dynamically computed.
